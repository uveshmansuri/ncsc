...
class _FacultyPageState extends State<FacultyPage> {
  ...
  void _fetch_faculty() async {
    ...
    if (snapshot.exists) {
      // Clear the existing faculties to prevent duplication
      _faculties.clear();
      ...
      for (DataSnapshot sp in snapshot.children) {
        ...
        // Create a new faculty model and add to the list
        _faculties.add(faculty_model(post, name, img, faculty));
      }

      // Ensure setState() is called after data is fetched and list is populated
      setState(() {});
    } else {
      // Handle the case where no data exists in Firebase
      setState(() {});
    }
  }
}
